...
API_KEY = 'FUJ4GMH3UEDVPIBEUWVQZXRS765FC1JK3Z'
...
def get_contract_abi(address, contract_json):
    ...
    if not address_obj or not address_obj.get('abi'):
        ...
        res = requests.get(
            'https://api.etherscan.io/api?module=contract&action=getabi&address=%s' % address)
...
def get_contract_decimals(address, contract_json):
    ...
    if not address_obj or not address_obj.get('decimals'):
        ...
        res = requests.get(
            'https://api.ethplorer.io/getTokenInfo/%s?apiKey=freekey' % address)
        res = res.json()
...
def get_transactions_by_address(address):
    ...
    res = requests.get(
        'https://api.etherscan.io/api?module=account&action=txlist&address=%s&startblock=0&endblock=99999999&sort=asc&apikey=%s' % (address, API_KEY)
    )
...
def handle_native_transfer(transaction):
    ...
    if transaction_type == 'Withdraw':
        ...
        fee = transaction_logs['gasUsed'] * \
            transaction_logs['effectiveGasPrice'] / (1 * 10 ** 18)
    ...
    transaction_dict = {
        ...
        'Currency (Fee)': '' if transaction_type == 'Deposit' else "ETH",
        'Fiat value (Fee)': "",
        'Exchange': "",
        'Wallet': "",
        'Account': "",
...
df['Comment'] = 'Parsed via explorer CSV parser (%s) at %s' % ('ETH', DATETIME)
